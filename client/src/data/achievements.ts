import { 
  FaMedal, 
  FaRunning, 
  FaChalkboardTeacher, 
  FaBiking,
  FaLaptopCode,
  FaWater,
  FaWheelchair,
  FaGlobeEurope
} from 'react-icons/fa';

export const achievements = [
  {
    title: 'Marathon Runner',
    description: 'Completed two London Marathons and half-marathons in Belgium and Germany.',
    icon: FaRunning
  },
  {
    title: '2x Hackathon Winner',
    description: 'Won 2 hackathons out of 7 competitions, showcasing problem-solving skills and technical expertise.',
    icon: FaMedal
  },
  {
    title: 'Elite Coach',
    description: 'Level 2 certified coach in both wheelchair rugby and table tennis, currently coaching at St Mary\'s University.',
    icon: FaChalkboardTeacher
  },
  {
    title: 'Paralympic Training',
    description: 'Trained with the GB Paratriathlon squad and England\'s wheelchair rugby team.',
    icon: FaBiking
  },
  {
    title: 'Web Studio Founder',
    description: 'Launched easywebs.uk, helping small businesses establish their online presence with modern web solutions.',
    icon: FaLaptopCode
  },
  {
    title: 'PADI Diver',
    description: 'Licensed free diver and scuba diver, exploring up to 30m underwater.',
    icon: FaWater
  },
  {
    title: 'Wheelchair Rugby',
    description: 'Current player for the Harlequins Wheelchair Rugby Team.',
    icon: FaWheelchair
  },
];
